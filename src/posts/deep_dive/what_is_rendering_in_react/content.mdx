...
React는 참 똑똑하다. 내 체감 상 굉장히 빠른 속도로 계산하고, 이전 DOM과의 차이를 비교하여 알아채고, 그 차이가 있는 경우에만 DOM을 업데이트한다. 
...
위의 케이스들 말고는 리액트에서 리렌더링을 유발하는 경우는 없다. 이런 이유로, 우리는 렌더링을 유발하기 위해 `Redux Toolkit`을 쓸 때, `React Redux` 패키지도 설치하는 것이다. 내가 애용하는 `zustand`는 위의 케이스에서 setter를 실행하는 케이스로 리렌더링을 유발하지 않을까 싶다.
...
우리가 react lifecycle을 공부할 때, 자주 보는 그림에 나와 있는 그 단어들이다. 

![Image source: github.com/wojtekmaj/react-lifecycle-methods-diagram](/posts/deep_dive/what_is_rendering_in_react/3.png)

<b>`렌더 단계(Render Phase)`</b>는 지금까지 계속 말하던 과정이다. 렌더링을 유발한 그 컴포넌트를 찾고, 호출하고, 계산하고, 그 반환값을 이전 반환값들과 비교하는 모든 과정들 말이다. 

<b>`커밋 단계(Commit Phase)`</b>는 렌더 단계에서 계산한 DOM을 유저가 볼 수 있도록 실제(real) DOM에 적용하는 것을 말한다. 이 단계 이후에 Reflow, Repaint 등의 브라우저 렌더링이 발생한다.
